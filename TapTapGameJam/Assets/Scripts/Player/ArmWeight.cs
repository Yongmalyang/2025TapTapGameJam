...
public class ArmWeight : MonoBehaviour
{
    ...
    public void AttatchSelfToArm(GameObject ConnectedBody, bool isFirstArm, bool isLeft)
    {
        ...
        //Debug.Log("my ratio: " + MyRatio + ", curArm: " + curArmLength);
        ...
        if (isFirstArm)
        {
            if (isLeft) finalAnchor = -(ConnectedBody.GetComponent<SpriteRenderer>().bounds.size.x + MyRatio) / 2.0f;
            else finalAnchor = (ConnectedBody.GetComponent<SpriteRenderer>().bounds.size.x + MyRatio) / 2.0f;
        }
        else
        {
            if (isLeft) finalAnchor = curArmLength - MyRatio;
            else finalAnchor = curArmLength + MyRatio;
        }
        */
    ...
    /*
    public float AttatchSelfToArm(GameObject ConnectedBody, float curArmLength, bool isFirstArm, bool isLeft)
    {
        Rigidbody2D ConnectedRB = ConnectedBody.GetComponent<Rigidbody2D>();
        ...
        // 붙일 접합면 정하기 (anchor)
        float MyRatio = GetComponent<SpriteRenderer>().bounds.size.x;
